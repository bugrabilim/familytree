...
| 35 | Katkı verici rolü | E | `ORDER = ["viewer","editor","admin"]` dizisine kademe sokmak = **her yetki kapısını** yeniden değerlendirmek. 🔸 **ön şart karşılandı**: `tests/auth-coverage.test.mts` 62 rotanın her HTTP yöntemini tek tek denetliyor; kademe eklenince envanter buradan okunur |
...
| 65 | Gelen posta kutusu (`bilgi@soylus.com` → `/admin/posta`) | webhook + imza + depo + ekran + yanıt | ✅ **çalışıyor** (2026-09-05) — MX + `RESEND_WEBHOOK_SECRET` + `ADMIN_ACCOUNT_IDS`; gövde için anahtarın okuma yetkisi gerekiyor. **Gelen posta ayrıca kendi adresine iletiliyor** (`INBOX_FORWARD_TO`) — sayfayı açmak zorunlu değil |

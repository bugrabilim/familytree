...
| 35 | Rol modeli — Yönetici + Üye | E | ✅ **yapıldı** — dört kademe (`viewer/contributor/editor/admin`) İKİYE indi: `yonetici` (ağacı kuran) doğrudan yazar ve önerilere karar verir; `uye` okur ve ÖNERİR. Eski adlar `normalizeRole` ile çevriliyor (`admin`→yönetici, gerisi→üye; kimse yetki kazanmıyor), göç betiği yok. Öneri motoru alan/ekleme/silme türlerini taşıyor. **Kalan:** ilişki değişikliği önerileri, öneriyi geri çekme, toplu karar, onayı geri alma, ve içerik depolarının (tarif/etkinlik/mektup) öneriye bağlanması |
...
| 39 | Haftalık soru serisi (Storyworth kadansı) | C | ✅ **yapıldı — teşhis değişti, değişmez KIRILMADI.** Eski satır "Storyworth için ayrı giriş kapısı; 'ağaçsız hesap' `accountId === treeId` değişmezini kırıyor" diyordu. Ölçüldü ve çürütüldü: (1) ağaçsız hesap zaten var, adı **üye** (`lib/tree-context.ts` üyeye ağaç kaydı sormuyor); (2) girişsiz yazma yüzeyi zaten var — `lib/public-routes.ts`te yedi tane, `/hikaye` dâhil; (3) değişmezi kırmanın kullanıcıya görünen faydası sıfır, bedeli `lib/tree-access.ts`teki "eşitlik sağlandıysa kayda bakmadan kurucu" satırını iki ad uzayı arasına yaymak. Eksik olan **kadanstı**: hikâye bağlantısı hiç gönderilmiyordu ve `nextPrompt`in gerekçesindeki "haftalık cron" hiç yazılmamıştı. Yapılan: `lib/story-series.ts` (saf `planWeekly`), `lib/story-store.ts` `createSeries`/`closeSeries`/`issueWeekly`/`markWeeklySent`, günlük cron'un içinde pazar günü dalı (Hobby'de üçüncü cron yuvası yok). Değişmez artık `tests/tree-identity-gate.test.mts` ile kilitli — kırmak isteyen önce o dosyayı okumak zorunda |
...
| 44 | Çevrimdışı yakalama + senkron | E | ✅ **yapıldı** — **yalnız mobilde.** İyimser kilitle (`x-base-version`) çakışıyor. 29'a bağlı. 🔸 **Ayrı bir not (2026-09-07, ürün sahibi):** "sistemi tamamen çevrimdışı çalıştırma" ayrı ve çok daha büyük bir iştir, sonra değerlendirilecek. 44 bunu KAPSAMIYOR: 44, çevrimdışıyken **yazma niyetini cihazda sıraya alıp** bağlantı gelince göndermektir; ağaç okuma, arama, harita, kitap hâlâ ağ ister. Tam çevrimdışı çalışma, ağacın tamamının cihaza inmesi + yerel bir okuma katmanı + gizlilik katmanının cihazda uygulanması demek — kapsamı 44'ün birkaç katı |
| 45 | Supabase Faz 4 — bcrypt + `users.json` emekliye | E | 🔸 **İki parçası bitti, biri kaldı.** ✅ **1a** senkron best-effort olmaktan çıktı (sıfırlama Auth'a yazılmadan tamamlanmıyor; bcrypt'ten bağımsız bir açığı da kapattı: senkron düşünce Auth'ta eski şifre kalıyordu). ✅ **1b** kurucunun bcrypt yedeği varsayılan kapalı — `AUTH_BCRYPT_FALLBACK=1` ile anında geri alınır, `SUPABASE_AUTH_LOGIN` kapalıyken zorla açık kalır (kilitlenme imkânsız); **üye girişleri kapsam dışı**, davetlilerin `auth.users` kaydı yok. ⏳ **2** `users.json`'ın kimlik kaynağı olmaktan çıkması — **tek geri dönüşü olmayan iş**, 1b canlıda birkaç gün bekledikten sonra. Kapı ölçülüyor: `GET /api/admin/phase4`; ayrıntı `docs/SUPABASE-GECIS.md` |
...
| 59 | Aile meclisi / fon kapsamı | ✅ **EVET** (2026-09-07, ürün sahibi — önerimin aksine). Artık karar değil **iş**. Uyarı kayda geçsin: uygulama içinde gerçek para hareketi (toplama/aktarma/saklama) finansal düzenlemeye girer ve ödeme kuruluşu lisansı ister. Bu yüzden kapsam **para taşımayan** kurguyla açılacak: aidat/katkı **kaydı**, borç-alacak defteri, karar ve oylama tutanağı — para transferi kullanıcının kendi bankası/IBAN'ı üzerinden, biz yalnız **kaydını** tutarız. Bant: C (yeni rota + görünüm). ✅ **yapıldı** (2026-09-07) — `types/council.ts` + `lib/council.ts` (saf: kuruş tamsayısı, IBAN düşüren metin temizliği, defter matematiği, tutanak dondurma), `lib/council-store.ts` (`council-<treeId>.json`, `mutateStore` ile kayıp yazma koruması), `app/api/family/council` (defter yazma `canEdit`, OY VERME `canPropose`; iyimser kilit defterin kendi damgasına karşı), `components/CouncilDialog.tsx` (⋮ → Aile meclisi; `publicView`'da hiç çizilmiyor). **IBAN KARARI: hiç saklanmıyor** — alan yok, serbest metne yazılan IBAN/kart numarası kaydedilmeden düşürülüyor (`stripIban`); ödeme bilgisini ağaç sahibi kendi kanalından paylaşır. Para hareketi yasağı kaynak düzeyinde kilitli: `tests/council-gate.test.mts` (ödeme sağlayıcısı/kart/bakiye kavramı ve dış uç adresi yasak, para alanları `…Kurus` tamsayısı, kapalı tutanak değiştirilemez/silinemez, meclis paylaşım bağlantısında ve `/embed`'de görünmez); birim testi `tests/council.test.mts` |
| 60 | Telefonla hikâye kaydı | ❌ **İPTAL** (2026-09-08, ürün sahibi). Daha önce "evet" idi; karar geri alındı. Hiçbir kod yazılmamıştı, dolayısıyla temizlenecek bir şey yok — iş numara/hesap açılmasını beklediği için hiç başlamamıştı. |
| 61 | Kuşak adı/rütbesi terminolojisi | ✅ **EVET** (2026-09-07, ürün sahibi — önerimin aksine). Artık karar değil **iş**. Türkçe kuşak adlandırması (ör. "yedi göbek" hattındaki basamak adları) kişiye **yazılan bir alan olarak değil, hesaplanan bir etiket olarak** üretilecek — `lib/relations.ts` zaten kan derecesi ve Türkçe akrabalık adı türetiyor, etiket oradan çıkar. Elle girilen alan eklenmez: elle girilen kuşak adı ilk ağaç düzenlemesinde yanlışa döner. Bant: B (saf lib + görünüm). ✅ **yapıldı** (2026-09-07) — `lib/generation.ts` (saf katman; ad merdiveninin bittiği yer `lib/relations.ts`'ten `NAMED_ANCESTOR_DEPTH`/`NAMED_DESCENDANT_DEPTH` ile ithal edilir, ötesi "N. kuşak ata/torun" sayısal biçimine düşer), görünüm: `PersonDrawer` kuşak rozeti + Yedi Göbek kartındaki göbek adları, i18n TR+EN `generation.*`. `Person`'a alan EKLENMEDİ; kuralı `tests/generation.test.mts` kaynak düzeyinde kilitliyor |
| 62 | Kalıcı arşiv taahhüdü | ✅ **KARAR: "sınırsız + her an tam dışa aktarım"** (2026-09-07, ürün sahibi). Önerim "X yıl" idi; ürün sahibi süre sınırı koymamayı seçti. Taahhüdün ayakta durmasını sağlayan şey #318 ile gelen **tek dosyalık HTML yedeği**: veri kullanıcının kendi cihazında da durduğu için "sınırsız", altyapımızın ömrüne bağlı bir söz olmaktan çıkıyor. ✅ **metne geçirildi** (2026-09-08): Kullanım Şartları'na "Arşiv ve dışa aktarım" paragrafı (TR+EN, `lib/i18n-dict.ts`) ve `docs/YEDEKLEME.md` §0 — orada taahhüdü bozabilecek üç somut yol da yazılı (dışa aktarımın koşula bağlanması, HTML yedeğinin dışarıya bağımlı hâle gelmesi, yeni `Person` alanının dışa aktarıma eklenmemesi) |
...
| 64 | Karar: şifreli belge kasası | ❌ **İPTAL** (2026-09-08, ürün sahibi). Karar bekliyordu, karar "hayır" oldu. Kırılma noktası kayda geçsin: anahtar kaybolursa belge kalıcı gider — kurtarabiliyorsak zaten şifreli değildir. İhtiyaç yeniden doğarsa önerim değişmedi: **imzalı, süreli erişim bağlantısı** (belge normal saklanır, bağlantı X saat sonra ölür); "bunu herkes görmesin"i karşılar, kalıcı kayıp riski getirmez. |

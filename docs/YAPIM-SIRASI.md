...
| 45 | Supabase Faz 4 — bcrypt + `users.json` emekliye | E | **Tek geri dönüşü olmayan iş.** 43, 46 ve 51 olmadan yapılmaz. Kapı ölçülüyor: `GET /api/admin/phase4`. 🔸 Ön koşullardan biri kapandı: **demo kimlik sisteminden çıkarıldı** (bir hesap değil, bir vitrin) — `lib/demo-account.ts` artık `users.json`a yazmıyor; üretimdeki eski satır elle silinecek, ayrıntı `docs/SUPABASE-GECIS.md` |

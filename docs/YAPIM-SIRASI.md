...
| 35 | Katkı verici rolü | E | 🔸 **A parçası yapıldı** (rol kademesi + sunucu kapıları + `addedBy` sahipliği). Kalan: **B** öneri deposu + onay akışı + bildirim, **C** arayüz (öner düğmesi, öneri paneli, üye seçeneği). Ekleyebildikleri: kişi, medya, tarif, etkinlik, mektup. Var olana dokunamaz; kendi eklediğini düzeltebilir, silemez |
